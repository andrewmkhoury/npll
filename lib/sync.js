...
export function add(opts, log = console.log) {
  ...
  try {
    ...
    if (destPm === 'yarn') {
      ...
      // Use the same direct package.json write used for yarn: pnpm add rejects
      // file: specs when the package appears in a pnpm catalog, and attempting
      // to update the full workspace lockfile can 401 on unrelated scoped packages.
      const pkgJsonPath = path.join(destDir, 'package.json');
      const pkg = JSON.parse(fs.readFileSync(pkgJsonPath, 'utf8'));
      if (pkg.dependencies?.[pkgName] === undefined) {
        pkg.devDependencies ??= {};
        pkg.devDependencies[pkgName] = fileRef;
      } else {
        pkg.dependencies[pkgName] = fileRef;
      }
      fs.writeFileSync(pkgJsonPath, JSON.stringify(pkg, null, 2));
      // --filter . scopes the install to this package only, avoiding workspace-wide
      // resolution that can 401 on unrelated scoped packages in the monorepo.
      run(destDir, 'pnpm', ['install', '--filter', '.', '--no-lockfile']);
    } else {
